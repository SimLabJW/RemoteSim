...
import threading
from .sim_models.thread_communicator import ThreadCommnuicator
...
class Simulator():
    ...
    def engine_register_entity(self) -> None:
        ...
        self.initializer_model = Initializer(instance_time = 0, destruct_time = Infinite,\
            name = "initializer_model", engine_name = self.engine_name)
        self.predictor_model = Predictor(instance_time = 0, destruct_time = Infinite,\
            name = "predictor_model", engine_name = self.engine_name)
        self.mover_model = Mover(instance_time = 0, destruct_time = Infinite,\
            name = "mover_model", engine_name = self.engine_name)
    ...
    def engine_coupling_relation(self) -> None:
        ...
        self.sm_engine.coupling_relation(self.thread_cm_model, "generate",\
            self.initializer_model, "generate")
        
        self.sm_engine.coupling_relation(self.initializer_model, "init_done", self.predictor_model, "init_done")
        self.sm_engine.coupling_relation(self.initializer_model, "init_done", self.mover_model, "init_done")
        self.sm_engine.coupling_relation(self.predictor_model, "pred_done", self.mover_model, "pred_done")
        self.sm_engine.coupling_relation(self.mover_model, "move_done", self.predictor_model, "move_done") 
